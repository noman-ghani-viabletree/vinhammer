...
class Fep_Role_To_Role_Block
  {
    ...
    function actions_filters()
    	{
			...
			add_filter( 'fep_filter_rest_users_args', array($this, 'hide_users' ) );
	...
	function fep_action_validate_form( $where, $errors, $fields )
		{
			...
			foreach( $block as $b ){
				...
				if( is_array( $to ) ){
					foreach( $to as $t ){
						if( get_current_user_id() != $t && ( 'fep_all' == $b['to_role'] ||  in_array( $b['to_role'], get_userdata( $t )->roles ) ) && apply_filters( 'fep_filter_add_error_for_rtr_block', true, $t ) ){
							$errors->add( 'pro_to' , sprintf(__('You cannot message to %s', 'front-end-pm'), fep_user_name( $t )));
							//return;
						}
					}
				} else {
					if( get_current_user_id() != $to && ( 'fep_all' == $b['to_role'] || in_array( $b['to_role'], get_userdata( $to )->roles ) ) && apply_filters( 'fep_filter_add_error_for_rtr_block', true, $to ) ){
	...
	function mr_newmessage_to_users( $can, $cap, $id ){
		...
		if( ! fep_can_send_to_max_recipients() )
		return false;
