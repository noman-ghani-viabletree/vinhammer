...
class FEP_Email_Parser {
    ...
    public function decode() {
        ...
        $this->date = isset( $this->decoded->headers['delivery-date'] ) ? $this->decoded->headers['delivery-date'] : '';
        ...
        if( 'body' === apply_filters( 'fep_email_piping_code_location', 'subject' ) ) {
			$this->piping_code = $this->get_string_between( $this->body(), $identifier[0], $identifier[1] );
		} else {
			$this->piping_code = $this->get_string_between( $this->subject(), $identifier[0], $identifier[1] );
		}
    ...
    public function date(){
		return $this->date ? gmdate( 'Y-m-d H:i:s', strtotime( $this->date ) ) : '';
	}
	...
	public function blog_id(){
		...
		return isset( $array[1] ) ? absint( $array[1] ) : '';
	}
    ...
    public function sender_id(){
        ...
        if( is_multisite() ) {
            return isset( $array[2] ) ? absint( $array[2] ) : '';
        } else {
            return isset( $array[1] ) ? absint( $array[1] ) : '';
        }
	}
