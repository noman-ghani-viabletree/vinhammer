...
class wpDiscuzFlagHelper {
    ...
    private static $_DOWN_VOTES = "down_voted";
    private static $_FLAGGINGS = "flagged";
    private static $_REPORT = "reported";
    ...
    public function __construct($dbManager, $options) {
        ...
        add_action("wpdiscuz_comment_flagged_email", [&$this, "flaggedActions"], 9);
    ...
    public function addReportFlag($output, $comment, $commentUser, $currentUser) {
        if (!empty($commentUser->ID) && !empty($currentUser->ID) && (int)$commentUser->ID === (int)$currentUser->ID) {
            return $output;
        }

        $notAllowedCommentTypes = apply_filters("wpdiscuz_rf_exclude_comment_types", [WpdiscuzCore::WPDISCUZ_STICKY_COMMENT]);
        if ($this->options->showFlag && !in_array($comment->comment_type, $notAllowedCommentTypes) &&
            !intval(get_comment_meta($comment->comment_ID, WpdiscuzCore::META_KEY_CLOSED, true))) {
            if ($currentUser && isset($currentUser->ID) && $currentUser->ID) {
                ...
                $output .= "<div style='$style' class='comment_flag $class wc_flag_tooltipster wpf-cta wpd-field-desc'  wpd-tooltip-position='left' wpd-tooltip-size='long' id='fc_{$comment->comment_ID}' wpd-tooltip='$title'>";
                $output .= "<i class='fas fa-flag'></i>";
                $output .= "</div>";
            }
        }
    ...
    public function flaggedActionsEmail($commentId, $report, $message) {
        ...
        // TODO
        do_action("wpdiscuz_rf_" . self::$_REPORT, [
            "email_to" => $to,
            ...
            if ($this->dbManager->moveVotedComments($comment->comment_ID, $this->options->autoModerateCommentType)) {
                if ($this->options->notifyAdmin == 1) {
                    if ($this->dbManager->updateCommentType($comment->comment_ID)) {
                        $this->getEmailData($comment, self::$_DOWN_VOTES);
                    }
                } else {
    ...
    public function getEmailData($comment, $type) {
        ...
        // TODO
        do_action("wpdiscuz_rf_" . $type, [
            "email_to" => $this->options->flaggedMailTo,
            "email_subject" => $subject,
            "email_message" => $body,
            "comment" => $comment,
            "user" => $currentUser,
