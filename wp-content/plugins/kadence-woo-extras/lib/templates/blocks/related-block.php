...
/**
 * Renders the `kadence-wootemplate-blocks/related` block on the server.
 *
 * @param array    $attributes Block attributes.
 * @param string   $content    Block default content.
 * @param WP_Block $block      Block instance.
 */
function kadence_wootemplate_render_related_block( $attributes, $content, $block ) {
	...
	$content      = '';
	$output_css   = '';
	$wrap_classes = 'kwt-related-wrap kwt-related-' . ( ! empty( $attributes['uniqueID'] ) ? $attributes['uniqueID'] : $post_ID );
	if ( is_singular( 'product' ) && is_main_query() && get_queried_object_id() === $post_ID ) {
		$wrap_classes .= ' kwt-related-single';
		...
		if ( ! empty( $attributes['columns'][0] ) ) {
			$wrap_classes .= ' kwt-related-columns-' . $attributes['columns'][0];
		}
		if ( ! empty( $attributes['columns'][1] ) ) {
			$wrap_classes .= ' kwt-related-tablet-columns-' . $attributes['columns'][1];
		}
		if ( ! empty( $attributes['columns'][2] ) ) {
			$wrap_classes .= ' kwt-related-mobile-columns-' . $attributes['columns'][2];
		}
		ob_start();
	...
	$css = kadence_wootemplate_render_related_output_css( $attributes );
	if ( ! empty( $css ) ) {
...
/**
 * Renders the `kadence-wootemplate-blocks/related` block css.
 *
 * @param array  $attributes Block attributes.
 * @param string $unique_id  Block Unique Id.
 */
function kadence_wootemplate_render_related_output_css( $attributes ) {
	...
	$style_id  = 'kwt-related-' . esc_attr( $unique_id );
	$css = Kadence_Woo_CSS::get_instance();
	if ( $css->has_styles( $style_id ) ) {
		return '';
	}
	$css->set_style_id( $style_id );
	$css->set_selector( '.kwt-related-wrap.kwt-related-' . $unique_id );
	$css->render_measure( $attributes, 'padding', 'padding' );
	$css->render_measure( $attributes, 'margin', 'margin' );
	$css->set_selector( '.kwt-related-wrap.kwt-related-' . $unique_id . ' .related.products ul.products li.product .woocommerce-loop-product__title' );
	$css->render_typography( $attributes, 'typography' );
	$css->render_color( $attributes, 'color', 'color' );
	$css->set_selector( '.kwt-related-wrap.kwt-related-' . $unique_id . ' .related.products ul.products li.product' );
	$css->render_color( $attributes, 'background', 'background' );
	// Columns.
	if ( ! empty( $attributes['columns'][0] ) ) {
		$css->set_selector( '.kwt-related-wrap.kwt-related-' . $unique_id . ' .related.products ul.products' );
